...
(function (root, factory) {
	...
	function Bootoast(options) {
		...
		this.$el = $('<div class="d-flex' + pluginName + ' alert alert-' + this.typeFor(this.settings.type) + '">' +
            '<span class="fa fa-' + this.settings.icon + ' mx-1"></span>' +
            '<span class="bootoast-alert-container"><span class="bootoast-alert-content">' + this.content + '</span></span></div>');
	...
	$.extend(Bootoast.prototype, {
		...
		init: function () {
			...
			if (this.settings.dismissible === true) {
				this.$el
					...
					.prepend('<button type="button" class="btn close" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></button>')
		...
		positionFor: function (position) {
			...
			return this.positionSinonym[positionCamel] || 'bottom-left';
		},
